...
for i in range(len(grid)):
    energized = set()
    dfs(i, 0, 0, set(), energized, grid)
    res = max(len(energized), res)
    energized = set()
    dfs(i, len(grid[0])-1, 2, set(), energized, grid)
    res = max(len(energized), res)

for j in range(len(grid[0])):
    energized = set()
    dfs(0, j, 1, set(), energized, grid)
    res = max(len(energized), res)
    energized = set()
    dfs(len(grid)-1, j, 3, set(), energized, grid)
    res = max(len(energized), res)
